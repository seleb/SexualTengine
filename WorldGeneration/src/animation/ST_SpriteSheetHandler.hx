...
class ST_SpriteSheetHandler
{
	...
	public static function getSpriteArray(_imgWidth:Int, _imgHeight:Int, _spriteWidth:Int, _spriteHeight:Int, _frames:Array<Int>):Array<Rectangle>
	{
		...
		/*
		 *  If the i * the sprite width is greater than the image width then subtract the image width * the number of rows down the image is
		...
		for (i in _frames) {
			...
			(i*_spriteWidth) >= ((Math.floor(_imgWidth/_spriteWidth))*_spriteWidth) ? ((i * _spriteWidth) - Math.floor((i*_spriteWidth)/((Math.floor(_imgWidth/_spriteWidth))*_spriteWidth))*_imgWidth) : (i * _spriteWidth),
