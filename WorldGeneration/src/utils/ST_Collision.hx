		...
		if (collRect.width == 0 && collRect.height == 0) {
			...
			for (x in 0 ... Math.round(collRect.width)) {
				for (y in 0 ... Math.round(collRect.height)) {
					...
					//bx += obj1.animation.getXY();
					//ST_SpriteSheetHandler.getSpriteArray(
					var a1:Int = obj1.animation.getBitmapData().getPixel32(bx, by);
					bx = Math.round(x + collRect.x - obj2.x);
					by = Math.round(y + collRect.y - obj2.y);
					var a2:Int = obj2.getBitmap().bitmapData.getPixel32(bx, by);
					//var a2:UInt = obj2.animation.getBitmapData().getPixel32(bx, by);
